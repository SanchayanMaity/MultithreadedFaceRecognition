...
//const char *faceRecognitionAlgorithm = "FaceRecognizer.Fisherfaces";
const char *faceRecognitionAlgorithm = "FaceRecognizer.Eigenfaces";
...
const char *faceCascadeFilename = "/home/root/lbpcascade_frontalface.xml";     // LBP face detector.
...
const char *eyeCascadeFilename1 = "/home/root/haarcascade_eye.xml";               // Basic eye detector for open eyes only.
const char *eyeCascadeFilename2 = "/home/root/haarcascade_eye_tree_eyeglasses.xml"; // Basic eye detector for open eyes if they might wear glasses.
...
const int DESIRED_CAMERA_HEIGHT = 400;
...
const char *windowName = "Toradex Face Recognition Demo";   // Name shown in the GUI window.
...
// Defined in recognition.cpp
// Below four are global variables 
extern vector<Mat> preprocessedFaces;
extern vector<int> faceLabels;
extern string facerecAlgorithm;
extern Ptr<FaceRecognizer> faceRecognitionModel;
//extern std::mutex trainingSignalMutex;
//extern std::condition_variable waitForTrainingSignal;
//extern std::mutex modelUpdationMutex;

extern pthread_mutex_t pmodelUpdationMutex;
extern pthread_mutex_t ptrainingSignalMutex;
extern pthread_cond_t pwaitForTrainingSignal;
extern bool canUseFaceRecognitionModel;
extern int trackTrainingCompletion;

vector<string> namesOfPersons;			// Will hold the names of the people registered by matching with identity number of face recognition algorithm
...
void initWebcam(VideoCapture &videoCapture, int cameraNumber)
{
    ...
    try {   // Surround the OpenCV call by a try/catch block so we can give a useful error message!
		...
		videoCapture.set(CV_CAP_PROP_FRAME_HEIGHT, 400);
		/*------------------------------------------------------------*/
    } catch (cv::Exception &e) {}
...
void onMouse(int event, int x, int y, int, void*)
{
    ...
    if (isPointInRect(pt, m_rcBtnAdd)) {
		...
		// Do nothing for Debug button press			
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
    ...
    vector<int> labelsForFaces;
    Mat old_prepreprocessedFace;
    double old_time = 0;
	bool unknownPerson = false;
    ...
    while (true) {
        ...
        if (m_mode == MODE_DETECTION) {
            ...
            if (gotFaceAndEyes) {
				...
				// Add name addition logic				
				string tempName;
				string nameOfPerson; 
				if (namesOfPersons.empty())	
				{	
					cout << "Enter your name: ";
					cin >> nameOfPerson; 
					namesOfPersons.push_back(nameOfPerson);
				}
				else if ((m_selectedPerson + 1) > namesOfPersons.size())
				{						
					cout << "Enter your name: ";
					cin >> nameOfPerson;
					namesOfPersons.push_back(nameOfPerson);											
                ...
                if ((imageDiff > CHANGE_IN_IMAGE_FOR_COLLECTION) && (timeDiff_seconds > CHANGE_IN_SECONDS_FOR_COLLECTION)) {
                    ...
                    labelsForFaces.push_back(m_selectedPerson);
                    labelsForFaces.push_back(m_selectedPerson);
            ...
            if (strcmp(faceRecognitionAlgorithm, "FaceRecognizer.Fisherfaces") == 0) {
            ...
            if (m_numPersons < 1 || preProcessedFaces.size() <= 0 || preProcessedFaces.size() != labelsForFaces.size()) {
            ...
            if (haveEnoughData) {
                ...
                //model = learnCollectedFaces(preProcessedFaces, labelsForFaces, faceRecognitionAlgorithm);
				//std::lock_guard<std::mutex> lk(trainingSignalMutex);
				pthread_mutex_lock(&ptrainingSignalMutex);
				preprocessedFaces = preProcessedFaces;
				faceLabels = labelsForFaces;
				facerecAlgorithm = faceRecognitionAlgorithm;				
				pthread_mutex_unlock(&ptrainingSignalMutex);
				//waitForTrainingSignal.notify_one();	
				pthread_cond_signal(&pwaitForTrainingSignal);								
            ...
            if (gotFaceAndEyes && (preProcessedFaces.size() > 0) && (preProcessedFaces.size() == labelsForFaces.size())) {				
				if (canUseFaceRecognitionModel)
				{
		            // Generate a face approximation by back-projecting the eigenvectors & eigenvalues.
		            Mat reconstructedFace;
					//std::lock_guard<std::mutex> guard(modelUpdationMutex);				
					pthread_mutex_lock(&pmodelUpdationMutex);													
		            reconstructedFace = reconstructFace(faceRecognitionModel, preprocessedFace);					
		            ...
		            if (similarity < UNKNOWN_PERSON_THRESHOLD) {
						...
						//std::lock_guard<std::mutex> guard(modelUpdationMutex);
						pthread_mutex_lock(&pmodelUpdationMutex);
		                identity = faceRecognitionModel->predict(preprocessedFace);
						pthread_mutex_unlock(&pmodelUpdationMutex);;
					...
					// Display on frame instead of console
					if (!namesOfPersons.empty() && (!unknownPerson))
					{	// Avoid raising an exception in case namesOfPersons is empty
						identityString = "Identity: " + outputStr + ". Similarity: " + toString(similarity) + ". " + namesOfPersons.at(identity);		
					}
					else
					{
						identityString = "Identity: " + outputStr + ". Similarity: " + toString(similarity);
						unknownPerson = false;
					}
	
		        	drawString(displayedFrame, identityString, Point(BORDER, -BORDER - 2 - 34), CV_RGB(0,0,0));         // Black shadow
		        	drawString(displayedFrame, identityString, Point(BORDER + 1, -BORDER - 1 - 34), CV_RGB(0,255,0));   // Green text
            ...
            labelsForFaces.clear();
            old_prepreprocessedFace = Mat();
			namesOfPersons.clear();
        ...
        if (m_mode >= 0 && m_mode < MODE_END) {			
			...
			else if (trackTrainingCompletion == 1)
				{
					 string modeStr = "MODE: " + string(MODE_NAMES[m_mode]) + "." + " Training Process ongoing";
		        	drawString(displayedFrame, modeStr, Point(BORDER, -BORDER-2 - rcHelp.height), CV_RGB(0,0,0));       // Black shadow
		        	drawString(displayedFrame, modeStr, Point(BORDER+1, -BORDER-1 - rcHelp.height), CV_RGB(0,255,0));   // Green text
				}
			else if (trackTrainingCompletion == 2)
				{
					string modeStr = "MODE: " + string(MODE_NAMES[m_mode]) + "." + " Training Process completed";
		        	drawString(displayedFrame, modeStr, Point(BORDER, -BORDER-2 - rcHelp.height), CV_RGB(0,0,0));       // Black shadow
		        	drawString(displayedFrame, modeStr, Point(BORDER+1, -BORDER-1 - rcHelp.height), CV_RGB(0,255,0));   // Green text
					trackTrainingCompletion = 0;		// From next frame, do not display training procedure status
				}
        }		
...
int main(int argc, char *argv[])
{
	...
	//std::thread threadHandle(learnCollectedFaces);	
	pthread_create(&trainingThread, NULL, learnCollectedFaces, NULL);
