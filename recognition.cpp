...
Ptr<FaceRecognizer> faceRecognitionModel;

// Mutex used for controlling updation of Face Recognizer Model
//std::mutex modelUpdationMutex;
pthread_mutex_t pmodelUpdationMutex = PTHREAD_MUTEX_INITIALIZER;

// Mutex used in conjunction with condition variable "waitForTrainingSignal"
//std::mutex trainingSignalMutex;
pthread_mutex_t ptrainingSignalMutex = PTHREAD_MUTEX_INITIALIZER;
// Condition variable on which training thread waits
//std::condition_variable waitForTrainingSignal;
pthread_cond_t pwaitForTrainingSignal = PTHREAD_COND_INITIALIZER;
bool canUseFaceRecognitionModel = false;		
int trackTrainingCompletion = 0;	
